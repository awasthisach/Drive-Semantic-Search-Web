...
export default function App() {
          ...
          if (driveData.files.length > 0 || driveData.folders.length > 0) {
            ...
            try {
              await saveDriveMetaSnapshot(driveData.files, driveData.folders, {
                corpus: String(savedCorpus || 'user'),
                sharedDriveId: savedDriveId,
                truncated: driveData.truncated,
              });
            } catch (e) {
              console.warn('Drive meta persist failed', e);
            }
          }
        } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result) {
        ...
        try {
          const driveData = await fetchGoogleDriveData(result.accessToken, driveFileTypeFilter, 40, driveCorpus, sharedDriveId || undefined);
          setDriveTruncated(Boolean(driveData.truncated));
          if (driveData.files.length > 0 || driveData.folders.length > 0) {
            setFiles(prev => {
              const driveIds = new Set(driveData.files.map(f => f.id));
            ...
            try {
              await saveDriveMetaSnapshot(driveData.files, driveData.folders, {
                corpus: String(driveCorpus || 'user'),
  ...
  const handleSyncGoogleDrive = async (
    ...
    const runFetch = async (tok: string) => {
      const driveData = await fetchGoogleDriveData(tok, typeToUse, 40, corpus, dId || undefined);
      setDriveTruncated(Boolean(driveData.truncated));
      setFiles(prev => {
        const driveIds = new Set(driveData.files.map(f => f.id));
        const remaining = prev.filter(f => !driveIds.has(f.id) && !f.isGoogleDriveItem);
  ...
  const handleToggleOffline = async (id: string) => {
    ...
    if (file.isGoogleDriveItem) {
      ...
      try {
        ...
        try { sha = await sha256Blob(blob); } catch { /* optional */ }
        const storeName = downloadName ? file.name.replace(/\.[^.]+$/, '') + downloadName : file.name;
        ...
        const hashById = new Map(offlineMeta.filter(m => m.sha256).map(m => [m.id, 'sha256:' + m.sha256!]));
        setFiles(prev => prev.map(f => {
          if (f.id === id) {
            return { ...f, isOffline: true, contentHash: sha ? ('sha256:' + sha) : f.contentHash, size: blob.size || f.size };
          }
        ...
        const hashLabel = sha ? (isExport ? ' (SHA-256 of offline export)' : ' (SHA-256)') : '';
        const evictMsg = evictedIds.length ? ' · Evicted ' + evictedIds.length + ' older pin(s)' : '';
    ...
    showDriveToast('Marked offline (local index only)');
  };
  ...
  const handleAddVaultFile = async (file: VaultFile) => {
    ...
    try { await saveVaultFile(file); } catch (e) {
  ...
  const handleDeleteVaultFile = async (id: string) => {
    ...
    try { await removeVaultFile(id); } catch (e) {
